...
#
# Usage:
#   server = MorkServer.MorkServer()
#   serve!(server)         # blocks until stop command received
#
# NOTE: this used to be a `"""..."""` docstring, but it collided with the
# enclosing module being also named `MorkServer` — Julia attached the doc
# to the struct then tried to use Type{MorkServer} as a Module, failing.
# Comments avoid the binding issue entirely. The same content lives in
# MorkServer's module-level docstring (see src/MorkServer.jl).
