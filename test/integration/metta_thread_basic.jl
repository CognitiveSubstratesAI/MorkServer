...
"""
    ...
    @test r2.status == 200
    @test wait_eq(
        "$BASE/status/$(enc("(def metta_thread_basic \$step \$p \$t)"))", "pathClear"
    )
...
"""
    ...
    @test r3.status == 200
    @test wait_eq(
        "$BASE/status/$(enc("(exec (metta_thread_basic \$priority) \$p \$t)"))", "pathClear"
    )
    ...
    @test r4.status == 200
    finished = wait_eq(
        "$BASE/status/$(enc("(exec metta_thread_basic)"))", "pathClear", 20.0
    )
    @test finished
    ...
    @test r5.status == 200
    finished2 = wait_eq(
        "$BASE/status/$(enc("(exec (metta_thread_basic state))"))", "pathClear", 15.0
    )
    @test finished2
